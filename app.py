...
const kindLabel={none:"None",input:"vMix Input",program:"Program (PGM)",preview:"Preview (PVW)",master:"Master",busA:"Bus A",busB:"Bus B",busC:"Bus C",busD:"Bus D",busE:"Bus E",busF:"Bus F",busG:"Bus G"};
...
function liveFor(ch){if(["input","program","preview"].includes(ch.kind)){const number=ch.kind==="program"?vmix.active:ch.kind==="preview"?vmix.preview:ch.input_number;const x=vmix.inputs.find(i=>ch.kind==="input"?(i.key===ch.input_key||(!ch.input_key&&i.number===number)):i.number===number);return x?{volume:x.volume,meter:x.meter}:{}}const x=vmix.outputs?.[ch.kind.toLowerCase()]||{};return x}
function drawChannels(){const root=$("channels"),bank=config.controllers[selectedController]?.bank_start||0;root.innerHTML="";config.channels.forEach((ch,i)=>{const live=liveFor(ch),fixed=ch.kind==="input",tr=document.createElement("tr");if(i>=bank&&i<bank+8)tr.className="in-bank";tr.innerHTML=`<td class="channel-number">${i+1}</td><td><select data-f="kind">${options(kinds,ch.kind,v=>kindLabel[v]||v)}</select></td><td><select data-f="input_key" ${fixed?"":"disabled"}>${inputOptions(ch.input_key)}</select></td><td><input data-f="input_number" type="number" min="0" value="${ch.input_number||0}" ${fixed?"":"disabled"}></td><td><input data-f="label_override" value="${esc(ch.label_override||"")}"></td><td><input data-f="follow_input_name" type="checkbox" ${ch.follow_input_name!==false?"checked":""}></td><td><span class="swatch" style="background:${ch.strip_color}"></span><select data-f="strip_color">${options(colors,ch.strip_color)}</select></td><td class="live">${live.volume==null?"-":Number(live.volume).toFixed(1)}</td><td class="live">${live.meter==null?"-":Number(live.meter).toFixed(2)}</td>`;tr.querySelectorAll("[data-f]").forEach(el=>el.oninput=()=>{const f=el.dataset.f;ch[f]=el.type==="checkbox"?el.checked:el.type==="number"?Number(el.value):el.value;if(f==="strip_color"||f==="kind")drawChannels()});root.appendChild(tr)});$("input-count").textContent=`${config.channel_count} logical channels · ${vmix.inputs.length} vMix inputs · bank ${bank+1}-${Math.min(bank+8,config.channel_count)}`}
function fillMaster(){const m=config.master_fader;m.kind=m.kind||"master";$("master_kind").innerHTML=options(kinds,m.kind,v=>kindLabel[v]||v);$("master_input_key").innerHTML=inputOptions(m.input_key);$("master_label").value=m.label_override||"Master";$("master_color").innerHTML=options(colors,m.strip_color||"white");[["master_kind","kind"],["master_input_key","input_key"],["master_label","label_override"],["master_color","strip_color"]].forEach(([id,key])=>$(id).oninput=()=>m[key]=$(id).value)}
